...
{
    [TestClass]
    public class InMemoryDatabaseTestsSample3 : InMemoryDatabaseTests
    {
        ...
        {
            var post = new Post { Title = "InMemoryDatabaseTestsSample3", Author = "Moi", Content = "DoSomething1" };
            Session.Save(post);
            Check.That(Session.Query<Post>().Count()).IsEqualTo(1);
            Check.That(Session.Query<Post>().Single().Title).IsEqualTo("InMemoryDatabaseTestsSample3");
        ...
        {
            var post = new Post { Title = "InMemoryDatabaseTestsSample3", Author = "Moi", Content = "DoSomething2" };
            Session.Save(post);
            Check.That(Session.Query<Post>().Count()).IsEqualTo(1);
            Check.That(Session.Query<Post>().Single().Title).IsEqualTo("InMemoryDatabaseTestsSample3");
        ...
        {
            var post = new Post { Title = "InMemoryDatabaseTestsSample3", Author = "Moi", Content = "DoSomething3" };
            Session.Save(post);
            Check.That(Session.Query<Post>().Count()).IsEqualTo(1);
            Check.That(Session.Query<Post>().Single().Title).IsEqualTo("InMemoryDatabaseTestsSample3");
